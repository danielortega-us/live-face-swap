...
PREVIEW = None
PREVIEW_MAX_HEIGHT = 1600
PREVIEW_MAX_WIDTH = 1200
PREVIEW_DEFAULT_WIDTH = 1600
PREVIEW_DEFAULT_HEIGHT = 900
...
def init(start: Callable[[], None], destroy: Callable[[], None], pw: int, ph: int) -> ctk.CTk:
    global ROOT, PREVIEW

    ROOT = create_root(start, destroy, pw, ph)
...
def create_root(start: Callable[[], None], destroy: Callable[[], None], pw: int, ph: int) -> ctk.CTk:
...
def webcam_preview(root: ctk.CTk, camera_index: int, pw: int, ph: int):
    if not modules.globals.map_faces:
        if modules.globals.source_path is None:
            # No image selected
            return
        create_webcam_preview(camera_index, pw, ph)
    else:
        modules.globals.souce_target_map = []
...
def create_webcam_preview(camera_index: int, pw: int, ph: int):
    global preview_label, PREVIEW

    camera = cv2.VideoCapture(camera_index)
    camera.set(cv2.CAP_PROP_FRAME_WIDTH, pw)
    camera.set(cv2.CAP_PROP_FRAME_HEIGHT, ph)
    camera.set(cv2.CAP_PROP_FPS, 60)

    preview_label.configure(width=pw, height=ph)
